...
> Durum: **ONAYLANDI — Faz 8 kesinti kuralları Şema ve Repository tamamlandı**
> Tarih: 22.07.2026
> Onay kaydı: Kullanıcı, önerilen 11 varsayımın tamamını onaylayarak Faz 0'ın başlatılmasını istedi.
> Aktif uygulama sınırı: Faz 5–7 kapandı. Kullanıcı `Docs/RFC-F8-01-kesinti-kurallari.md` içindeki on varsayımın tamamını onayladı. Faz 8 kesinti kuralları Dilim 1 Domain Çekirdeği ile Dilim 2 Şema ve Repository tamamlandı. Migration yerel PostgreSQL'e uygulandı; 1 legacy teminat kuralı backfill edildi, mevcut 5 hakediş ve 0 manuel kesinti hareketi değişmedi, application üretilmedi. Sıradaki bağımsız dilim Önizleme ve Uygulama servis/action katmanıdır.
