...
export function StockMovementSurface({
  ...
  return (
    <section className="mx-auto mb-4 grid max-w-7xl gap-4">
      <header className="rounded-[var(--radius-panel)] border border-[var(--grid-border)] bg-[var(--surface-container-lowest)] p-5"><p className="text-xs font-semibold uppercase tracking-wide text-[var(--primary)]">Normalize stok hareketi</p><div className="mt-2 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-xl font-semibold">Depo ve Şantiye Hareketleri</h2><p className="mt-1 text-sm text-[var(--on-surface-variant)]">Depolar arası transfer ve stok kontrollü şantiye çıkışı; taslak, kesinleştirme, iptal ve audit yaşam döngüsü.</p></div><div className="flex gap-2"><button className={buttonClass} disabled={!canMutate} onClick={() => startCreate("Depo Transferi")} type="button">Yeni Transfer</button><button className={buttonClass} disabled={!canMutate} onClick={() => startCreate("Şantiye Çıkışı")} type="button">Yeni Şantiye Çıkışı</button></div></div></header>
      {errors.length > 0 ? <div className="rounded-[var(--radius-panel)] border border-[var(--status-cancelled)] p-3 text-sm text-[var(--status-cancelled)]"><p className="font-semibold">Stok hareketi tamamlanamadı</p><ul className="mt-2 list-disc pl-5">{errors.map((error) => <li key={error}>{error}</li>)}</ul></div> : null}
      ...
      {form ? <article className="rounded-[var(--radius-panel)] border border-[var(--grid-border)] bg-[var(--surface-container-lowest)] p-4"><h3 className="text-sm font-semibold">{form.movementType}</h3><div className="mt-4 grid gap-3 md:grid-cols-3">
