  ...
  return (
    <section className="mx-auto mt-4 grid max-w-7xl gap-4">
      <header className="rounded-ui-panel border border-divider bg-surface-raised p-5">
        <p className="text-xs font-semibold uppercase tracking-wide text-brand-primary">Ekipman ve KKD takibi</p>
        <div className="mt-2 flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-xl font-semibold">Personel Zimmetleri</h2><p className="mt-1 text-sm text-content-subtle">Personel bazında ekipman, elektronik, demirbaş ve KKD teslim/iade yaşam döngüsü.</p></div><button className={buttonClass} disabled={!canMutate} onClick={startAssign} type="button">Yeni Zimmet</button></div>
      </header>
      {errors.length > 0 ? <div className="rounded-ui-panel border border-[var(--ds-danger)] p-3 text-sm text-[var(--ds-danger)]"><p className="font-semibold">Zimmet işlemi tamamlanamadı</p><ul className="mt-2 list-disc pl-5">{errors.map((error) => <li key={error}>{error}</li>)}</ul></div> : null}
      ...
      {form ? <article className="rounded-ui-panel border border-divider bg-surface-raised p-4"><h3 className="text-sm font-semibold">Yeni Personel Zimmeti</h3><div className="mt-4 grid gap-3 md:grid-cols-3">
