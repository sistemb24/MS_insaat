...
export async function listConstructionProgressPaymentDetailsAction(id: string) {
  ...
  const payment = await prisma.constructionProgressPayment.findFirst({
    ...
    include: { measurementSheets: { include: { lines: { include: { contractItem: { select: { itemCode: true, description: true, unit: true } } }, orderBy: { lineNo: "asc" } } }, orderBy: { sheetNo: "asc" } }, extraWorks: { orderBy: [{ workDate: "asc" }, { documentNo: "asc" }] }, deductionMovements: { include: { ruleApplication: { select: { id: true } } }, orderBy: { movementDate: "asc" } }, deductionRuleApplications: { orderBy: [{ appliedAt: "asc" }, { ruleCode: "asc" }] }, financialMovements: { orderBy: { movementDate: "asc" } } },
  });
  if (!payment) return { ok: false as const, errors: ["Kümülatif hakediş bulunamadı."] };
  return { ok: true as const, data: {
    canApplyDeductionRules: ["admin", "accounting"].includes(result.scope.userRole),
    ...
    deductions: payment.deductionMovements.map((row) => ({ id: row.id, category: row.category, documentNo: row.documentNo, movementDate: row.movementDate.toISOString(), description: row.description, amount: Number(row.amount), vatAmount: Number(row.vatAmount), totalAmount: Number(row.totalAmount), ruleApplicationId: row.ruleApplication?.id ?? null })),
    deductionRuleApplications: payment.deductionRuleApplications.map((row) => ({ id: row.id, ruleKey: row.ruleKey, ruleCode: row.ruleCode, ruleName: row.ruleName, ruleRevisionNo: row.ruleRevisionNo, calculationType: row.calculationType, baseType: row.baseType, baseAmount: Number(row.baseAmount), rate: row.rate === null ? null : Number(row.rate), fixedAmount: row.fixedAmount === null ? null : Number(row.fixedAmount), minimumAmount: row.minimumAmount === null ? null : Number(row.minimumAmount), maximumAmount: row.maximumAmount === null ? null : Number(row.maximumAmount), taxMode: row.taxMode, taxRate: Number(row.taxRate), taxAmount: Number(row.taxAmount), netAmount: Number(row.netAmount), totalAmount: Number(row.totalAmount), appliedBy: row.appliedBy, appliedAt: row.appliedAt.toISOString(), updatedAt: row.updatedAt.toISOString() })),
...
export async function getConstructionProgressPaymentReportAction(id: string) {
  ...
  const [ledger, auditLogs] = await Promise.all([
    payment.accountingLink ? prisma.ledgerEntry.findFirst({ where: { tenantId: result.scope.tenantId, companyId: result.scope.companyId, periodId: result.scope.periodId, sourceType: "progress-payment", sourceId: payment.accountingLink.progressPaymentId }, select: { id: true, entryDate: true, documentNo: true, description: true, currency: true, status: true, debitTotal: true, creditTotal: true, lines: { orderBy: { lineNo: "asc" }, select: { id: true, lineNo: true, accountCode: true, accountName: true, debit: true, credit: true, description: true } } } }) : Promise.resolve(null),
  ...
  return { ok: true as const, data: {
    ...
    accounting: payment.accountingLink ? { progressPaymentId: payment.accountingLink.progressPaymentId, ledgerEntryId: ledger?.id ?? null, ledgerDocumentNo: ledger?.documentNo ?? null, ledgerStatus: ledger?.status ?? null, entryDate: ledger?.entryDate.toISOString() ?? null, description: ledger?.description ?? null, currency: ledger?.currency ?? payment.currency, debitTotal: ledger ? Number(ledger.debitTotal) : null, creditTotal: ledger ? Number(ledger.creditTotal) : null, lines: ledger?.lines.map((line) => ({ id: line.id, lineNo: line.lineNo, accountCode: line.accountCode, accountName: line.accountName, debit: Number(line.debit), credit: Number(line.credit), description: line.description })) ?? [] } : null,
  } };
}
...
export async function deleteConstructionProgressPaymentDetailAction(input: { progressPaymentId: string; detailId: string; detailType: "MEASUREMENT_SHEET" | "MEASUREMENT_LINE" | "EXTRA_WORK" | "DEDUCTION" | "FINANCIAL_MOVEMENT" }) {
  ...
  if (input.detailType === "DEDUCTION") {
    const ruleApplication = await prisma.constructionDeductionRuleApplication.findFirst({ where: { deductionMovementId: input.detailId, progressPaymentId: resolved.payment.id, tenantId: resolved.scope.tenantId, companyId: resolved.scope.companyId, periodId: resolved.scope.periodId }, select: { id: true } });
    if (ruleApplication) return { ok: false as const, errors: ["Kural kaynaklı kesinti hareketi manuel silinemez; ters kayıt yaşam döngüsü kullanılmalıdır."] };
  }
