...
describe("GET /api/stok-karti-ozeti", () => {
  beforeEach(() => vi.clearAllMocks());
  test("returns scoped stock card summary", async () => {
    ...
    listMock.mockResolvedValue({ ok: true, data: { rows: [{ Durum: "Aktif", "Minimum Miktar": "5" }, { status: "Pasif" }] } });
    const response = await GET(new Request("http://localhost/api/stok-karti-ozeti", { headers: { authorization: "Bearer test" } }));
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ ok: true, data: { totalCount: 2, statusCounts: { Aktif: 1, Pasif: 1 }, minimumConfiguredCount: 1 } });
    expect(authenticateMock).toHaveBeenCalledWith(expect.objectContaining({ requiredScopes: ["stock-cards"] }));
  });
  test("challenges missing bearer", async () => {
    ...
    const response = await GET(new Request("http://localhost/api/stok-karti-ozeti"));
