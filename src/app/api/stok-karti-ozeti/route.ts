...
export async function GET(request: Request) {
  const authResult = await authenticateBearerApiKey({ authorizationHeader: request.headers.get("authorization"), requiredScopes: ["stock-cards"], repository: apiKeyRepository });
  ...
  const result = await entityService.list({ scope: buildTenantScopeFromApiKey(authResult.data.apiKey), slug: "stok-kartlari" });
  ...
  const minimumConfiguredCount = result.data.rows.filter((row) => { const value = row["Minimum Miktar"] ?? row.minimumQuantity ?? row.minimumMiktar; return value !== undefined && value !== ""; }).length;
  return Response.json({ ok: true, data: { totalCount: result.data.rows.length, statusCounts, minimumConfiguredCount } });
}
