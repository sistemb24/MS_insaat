...
describe("GET /api/personel-ozeti", () => {
  beforeEach(() => vi.clearAllMocks());
  test("returns scoped employee role and status summary", async () => {
    ...
    listMock.mockResolvedValue({ ok: true, data: { rows: [{ Rol: "Şantiye", Durum: "Aktif" }, { role: "Muhasebe", status: "Pasif" }] } });
    const response = await GET(new Request("http://localhost/api/personel-ozeti", { headers: { authorization: "Bearer test" } }));
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ ok: true, data: { totalCount: 2, roleCounts: { Şantiye: 1, Muhasebe: 1 }, statusCounts: { Aktif: 1, Pasif: 1 } } });
    expect(authenticateMock).toHaveBeenCalledWith(expect.objectContaining({ requiredScopes: ["employees"] }));
  });
  test("challenges missing bearer", async () => {
    ...
    const response = await GET(new Request("http://localhost/api/personel-ozeti"));
