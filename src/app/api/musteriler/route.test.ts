...
describe("customer api route", () => {
  test("lists customer entity rows for the api key tenant scope", async () => {
    const apiKey = {
    ...
    const response = await GET(
      new Request("http://localhost/api/musteriler", {
    ...
    expect(entityRepositoryMock.read).toHaveBeenCalledWith({
      scope,
      definition: expect.objectContaining({ slug: "musteriler" }),
    });
  });
  ...
  test("returns a bearer challenge when authorization is missing", async () => {
    ...
    const response = await GET(new Request("http://localhost/api/musteriler"));
