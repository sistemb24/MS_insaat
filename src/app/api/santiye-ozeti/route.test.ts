...
describe("GET /api/santiye-ozeti", () => {
  beforeEach(() => vi.clearAllMocks());
  test("returns scoped site status summary", async () => {
    ...
    listMock.mockResolvedValue({ ok: true, data: { rows: [{ Durum: "Devam Ediyor" }, { status: "Tamamlandı" }] } });
    const response = await GET(new Request("http://localhost/api/santiye-ozeti", { headers: { authorization: "Bearer test" } }));
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ ok: true, data: { totalCount: 2, statusCounts: { "Devam Ediyor": 1, "Tamamlandı": 1 } } });
    expect(authenticateMock).toHaveBeenCalledWith(expect.objectContaining({ requiredScopes: ["projects"] }));
  });
  test("challenges missing bearer", async () => {
    ...
    const response = await GET(new Request("http://localhost/api/santiye-ozeti"));
