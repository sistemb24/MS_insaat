...
export async function GET(request: Request) {
  const authResult = await authenticateBearerApiKey({ authorizationHeader: request.headers.get("authorization"), requiredScopes: ["projects"], repository: apiKeyRepository });
  ...
  const result = await entityService.list({ scope: buildTenantScopeFromApiKey(authResult.data.apiKey), slug: "santiyeler" });
