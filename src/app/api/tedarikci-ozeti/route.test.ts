...
describe("GET /api/tedarikci-ozeti", () => {
  beforeEach(() => vi.clearAllMocks());
  test("returns scoped supplier status summary", async () => {
    ...
    const response = await GET(new Request("http://localhost/api/tedarikci-ozeti", { headers: { authorization: "Bearer test" } }));
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ ok: true, data: { totalCount: 2, statusCounts: { Aktif: 1, Pasif: 1 } } });
    expect(authenticateMock).toHaveBeenCalledWith(expect.objectContaining({ requiredScopes: ["suppliers"] }));
  });
  test("challenges missing bearer", async () => {
    ...
    const response = await GET(new Request("http://localhost/api/tedarikci-ozeti"));
