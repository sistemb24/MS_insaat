...
describe("GET /api/taseron-ozeti", () => {
  beforeEach(() => vi.clearAllMocks());
  test("returns scoped contractor status summary", async () => {
    ...
    listMock.mockResolvedValue({ ok: true, data: { rows: [{ Durum: "Aktif" }, { status: "Sözleşmesi Bitti" }] } });
    const response = await GET(new Request("http://localhost/api/taseron-ozeti", { headers: { authorization: "Bearer test" } }));
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ ok: true, data: { totalCount: 2, statusCounts: { Aktif: 1, "Sözleşmesi Bitti": 1 } } });
    expect(authenticateMock).toHaveBeenCalledWith(expect.objectContaining({ requiredScopes: ["contractors"] }));
  });
  test("challenges missing bearer", async () => {
    ...
    const response = await GET(new Request("http://localhost/api/taseron-ozeti"));
