...
export async function GET(request: Request) {
  const authResult = await authenticateBearerApiKey({ authorizationHeader: request.headers.get("authorization"), requiredScopes: ["contractors"], repository: apiKeyRepository });
  ...
  const result = await entityService.list({ scope: buildTenantScopeFromApiKey(authResult.data.apiKey), slug: "taseronlar" });
  ...
  return Response.json({ ok: true, data: { totalCount: result.data.rows.length, statusCounts } });
}
