...
describe("supplier api route", () => {
  test("lists supplier entity rows for the api key tenant scope", async () => {
    const apiKey = {
    ...
    const response = await GET(
      new Request("http://localhost/api/tedarikciler", {
  ...
  test("returns a bearer challenge when authorization is missing", async () => {
    ...
    const response = await GET(new Request("http://localhost/api/tedarikciler"));
