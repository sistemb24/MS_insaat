import { describe, expect, test, vi } from "vitest";

const authenticateBearerApiKeyMock = vi.hoisted(() => vi.fn());
const buildTenantScopeFromApiKeyMock = vi.hoisted(() => vi.fn());
const apiKeyRepositoryMock = vi.hoisted(() => ({ kind: "api-key-repo" }));
const documentRepositoryMock = vi.hoisted(() => ({
  kind: "document-repo",
  listFiles: vi.fn(),
  listFolders: vi.fn(),
  listTrashedFiles: vi.fn(),
}));

vi.mock("@/lib/prisma", () => ({ prisma: {} }));
vi.mock("@/lib/document-center-prisma-repository", () => ({
  createDocumentCenterPrismaRepository: vi.fn(() => documentRepositoryMock),
}));
vi.mock("@/lib/api-key-auth", () => ({
  authenticateBearerApiKey: authenticateBearerApiKeyMock,
  buildTenantScopeFromApiKey: buildTenantScopeFromApiKeyMock,
}));
vi.mock("@/lib/api-key-prisma-repository", () => ({
  createApiKeyPrismaRepository: vi.fn(() => apiKeyRepositoryMock),
}));

import { GET } from "./route";

describe("document api route", () => {
  test("lists scoped document metadata without storage access", async () => {
    const apiKey = {
      companyId: "company-demo-insaat",
      createdBy: "user-main",
      periodId: "period-2026",
      tenantId: "tenant-noa-demo",
    };
    const scope = {
      companyId: "company-demo-insaat",
      companyName: "API Company",
      licenseLabel: "API",
      periodId: "period-2026",
      periodLabel: "API",
      tenantId: "tenant-noa-demo",
      tenantName: "API Tenant",
      userId: "user-main",
      userName: "API Kullanıcısı",
      userRole: "viewer" as const,
    };
    const files = [
      {
        id: "file-1",
        tenantId: scope.tenantId,
        companyId: scope.companyId,
        periodId: scope.periodId,
        folderId: "folder-1",
        name: "sozlesme.pdf",
        kind: "PDF",
        mimeType: "application/pdf",
        sizeBytes: 1024,
        storageKey: "documents/file-1",
        storageProvider: "local",
        linkedModule: "ihale-yonetimi",
        linkedRecordId: "IH-0001",
        linkedRecordLabel: "Merkez Yapı İhalesi",
        deletedAt: undefined,
        createdBy: "user-main",
        updatedBy: "user-main",
        createdAt: "2026-07-13T08:00:00.000Z",
        updatedAt: "2026-07-13T08:00:00.000Z",
      },
    ];
    const folders: unknown[] = [];
    const trashedFiles: unknown[] = [];

    authenticateBearerApiKeyMock.mockResolvedValue({
      ok: true,
      data: { apiKey, bearerToken: "noa_live_test" },
    });
    buildTenantScopeFromApiKeyMock.mockReturnValue(scope);
    documentRepositoryMock.listFiles.mockResolvedValue(files);
    documentRepositoryMock.listFolders.mockResolvedValue(folders);
    documentRepositoryMock.listTrashedFiles.mockResolvedValue(trashedFiles);

    const response = await GET(
      new Request("http://localhost/api/dokumanlar", {
        headers: { Authorization: "Bearer noa_live_test" },
      }),
    );

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({
      ok: true,
      data: {
        fileCount: 1,
        folderCount: 0,
        trashedFileCount: 0,
        files,
        folders,
        trashedFiles,
      },
    });
    expect(authenticateBearerApiKeyMock).toHaveBeenCalledWith({
      authorizationHeader: "Bearer noa_live_test",
      requiredScopes: ["documents"],
      repository: apiKeyRepositoryMock,
    });
    expect(documentRepositoryMock.listFiles).toHaveBeenCalledWith({ scope });
    expect(documentRepositoryMock.listFolders).toHaveBeenCalledWith({ scope });
    expect(documentRepositoryMock.listTrashedFiles).toHaveBeenCalledWith({ scope });
  });

  test("returns a bearer challenge when authorization is missing", async () => {
    authenticateBearerApiKeyMock.mockResolvedValue({
      ok: false,
      errors: ["Bearer API anahtarı zorunludur."],
      status: 401,
    });

    const response = await GET(new Request("http://localhost/api/dokumanlar"));

    expect(response.status).toBe(401);
    expect(response.headers.get("www-authenticate")).toBe("Bearer");
  });
});
